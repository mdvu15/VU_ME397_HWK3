...
solar_cap_cost 			= 800 * 10**6      # $/GW
wind_cap_cost           = 1200 * 10**6     # $/GW
ESS_p_cap_cost 			= 200 * 10**6      # $/GW
ESS_e_cap_cost 			= 150 * 10**6      # $/GWh
...
def match_const(model, i):
    return model.solar[i]*model.cap['s_cap'] +  model.wind[i]*model.cap['w_cap'] + model.ESS_d[i] - model.ESS_c[i] - model.curt[i] - demand == 0   
...
status.write(filename='solar_wind_storage.json', format='json')
